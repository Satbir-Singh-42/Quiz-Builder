...
export default function QuizResults({ result }: QuizResultsProps) {
  ...
  const answersMap = result.answers.reduce((acc: Record<number, number>, curr: any) => {
        ...
        <CardContent className="p-6 md:p-8">
          <div className="flex flex-col items-center justify-center mb-8 py-8 bg-slate-50 rounded-lg">
            ...
            <div className="text-xl mb-3">
              You scored <span className="font-semibold">{result.score} out of {result.totalQuestions}</span>
            </div>
            <Badge
          ...
          <div className="space-y-6">
            {result.questions && result.questions.map((question: any, index: number) => {
              ...
              return (
                <div key={question.id} className="border border-gray-200 rounded-md p-4 shadow-sm bg-white">
