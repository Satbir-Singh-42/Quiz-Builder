...
export default function TakeQuizPage() {
  ...
  const [isCheckingPreviousAttempt, setIsCheckingPreviousAttempt] = useState(true);
  ...
  useEffect(() => {
    const checkPreviousAttempt = async () => {
      ...
      try {
        const response = await apiRequest("GET", `/api/results/check?participantId=${participantId}&quizId=${id}`);
        ...
        if (data.hasTakenQuiz) {
          toast({
            ...
            description: "You have already taken this quiz. Redirecting to quiz list.",
            variant: "default",
          });
